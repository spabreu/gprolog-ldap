...
LDAPMod **decifra_atomo(PlTerm *atomo, int arity, int mod_op)
{
  ...
  printf("[decifra_atomo] É um compound!\n");
      ...
      ret[i]->mod_op = mod_op; /* ignorado no caso do l_add */
      ret[i]->mod_type = strdup(Atom_Name(f[i]));
	...
	(char **) malloc ((a[i]+1)*sizeof(*ret[i]->mod_values));
      for (j = 0; j < a[i]; j++) {
	ret[i]->mod_values[j] = strdup(Rd_String_Check(l[i][j]));
...
#ifdef DEBUG
  printf("[l_del] deleting DN '%s'\n", dn);
  printf("[l_del] on connection ID %d\n", conn);
#endif
...
Bool l_entry(int conn, int msg) //, int *msg_out)
{
  ...
  if (Get_Choice_Counter() == 0) {
    result = ldap_first_entry(ldap_connections[conn], 
			      ldap_messages[msg].initial_msg);
    Create_Water_Mark (lcb_msgfree, (void*) msg);
  }
  else {
...
/* l_value */
Bool l_value(int conn, int msg, char *attr, int bin, char **value)
{
  char **attr_list;
  int i;
...
Bool l_dn(int conn, int msg, PlTerm *dn_list)
{
  char *dn;
...
Bool l_modify_add(int conn, char *dn, PlTerm to_add)
{
  int f, a, rc;
  PlTerm *atomo;
  LDAPMod **attrs;
  ...
  atomo = Rd_Compound_Check(to_add, &f, &a);

#ifdef DEBUG
  printf("[l_modify_add] FUNCTOR: %s\n", Atom_Name(f));
  printf("[l_modify_add] ARIDADE: %d\n", a);
#endif
  ...
  /* depois metemos a info no servidor */
  if ( (rc = ldap_modify_s(ldap_connections[conn], dn, attrs)) 
       != LDAP_SUCCESS) {
    Proldap_err(ldap_err2string(rc));
    ldap_mods_free(attrs, 1);
    return FALSE;
...
#ifdef DEBUG
  printf("[l_modify_add] Data added to the entry\n");
#endif
  ldap_mods_free(attrs, 1);
  return TRUE;
...
Bool l_modify_replace(int conn, char *dn, PlTerm to_replace)
{
  int f, a, rc;
  PlTerm *atomo;
  LDAPMod **attrs;
  ...
  atomo = Rd_Compound_Check(to_replace, &f, &a);

#ifdef DEBUG
  printf("[l_modify_replace] FUNCTOR: %s\n", Atom_Name(f));
  printf("[l_modify_replace] ARIDADE: %d\n", a);
#endif

  /* primeiro temos de "decifrar" o átomo */
  attrs = decifra_atomo(atomo, a, LDAP_MOD_REPLACE);
...
#ifdef DEBUG
  printf("[l_modify_replace] Data replaced on the entry\n");
#endif
  ldap_mods_free(attrs, 1);
  return TRUE;
...
Bool l_modify_del(int conn, char *dn, PlTerm to_delete)
{
  int f, a, rc;
  PlTerm *atomo;
  LDAPMod **attrs;
  ...
  atomo = Rd_Compound_Check(to_delete, &f, &a);

#ifdef DEBUG
  printf("[l_modify_del] FUNCTOR: %s\n", Atom_Name(f));
  printf("[l_modify_del] ARIDADE: %d\n", a);
#endif

  /* primeiro temos de "decifrar" o átomo */
  attrs = decifra_atomo(atomo, a, LDAP_MOD_DELETE);
...
#ifdef DEBUG
  printf("[l_modify_del] Data removed from the entry\n");
#endif
  ldap_mods_free(attrs, 1);
  return TRUE;
...
Bool l_modify_dn(int conn, char *dn, char *newdn)
{
  int rc;

  // int ldap_modrdn2_s(ld, dn, newrdn, deleteoldrdn)
  if ( (rc = ldap_modrdn2_s(ldap_connections[conn], dn, newdn, 1)) 
       != LDAP_SUCCESS) {
    Proldap_err(ldap_err2string(rc));
    return FALSE;
  }
...
/*
$Log$
Revision 1.1  2003/04/04 17:10:02  spa
Initial revision

Revision 1.20  2001/06/24 23:34:44  pedro
* início da construção da função l_value()
* as funções l_add() e l_modify_*() já não libertam os átomos (strdup)

Revision 1.19  2001/06/20 01:34:34  pedro
*** empty log message ***

Revision 1.18  2001/06/20 00:49:22  pedro
(l_modify): dividida em 3 -> l_modify_add(), l_modify_replace() e l_modify_del()

inserido código para a desalocação dos ldapmod's

Revision 1.17  2001/06/19 23:44:17  pedro
(l_dn): agora o DN já sai em forma de lista (permite unificação, etc.)

Revision 1.16  2001/06/19 18:18:44  pedro
adicionado o "header" da função l_modify()
